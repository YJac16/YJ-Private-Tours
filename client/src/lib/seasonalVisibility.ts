...
/**
 * Inclusive season check on month/day only (repeats every year).
 * Handles same-year ranges (Jun–Oct) and wrap ranges (Nov–May) if ever needed.
 */
...
/** Hermanus fallback when admin_meta is missing but slug matches. */
...
/**
 * Public listing / booking visibility.
 * Tours without a season are always visible (unless status hidden/draft).
 */
